...
impl<'tcx> UnsafetyIsolationCheck<'tcx>{
    ...
    pub fn start_generate_upg(&mut self) {
        ...
        let dot_graphs = self.generate_upg();
        render_dot_graphs(dot_graphs);
        // self.show_nodes();
    }

    pub fn start_check_doc(&self) {
            ...
            for (body_id, _) in vec{
                ...
                if function_unsafe{
                    ...
                    if !self.check_if_unsafety_doc_exists(def_id)  {
                        ...
                        println!("{:?}:Lack of unsafety doc: {:?}, visibility:{:?}.",index, self.tcx.def_span(def_id),visibility);
                    }
                }
                if block_unsafe {
    ...
    pub fn start_count_uig(&mut self) {
        let related_items = RelatedFnCollector::collect(self.tcx);
        let hir_map = self.tcx.hir();
            ...
            for (body_id, _) in vec{
                ...
                if block_unsafe {
                    ...
                    if callees.is_empty() {  // single node
                        // println!("caller {:?}: {:?} ", caller_type, def_id.clone());
                        Self::update_type_vec(&mut type_vec, 3, 3, false, false);
                    }
                    for callee in callees {
                        let callee_type = self.get_type(callee);
                        // println!("caller {:?}: {:?} ---- callee{:?}: {:?}", caller_type, def_id.clone(),callee_type, callee.clone());
                        ...
                        if !two_safety1 && !two_safety2 {
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, only1, only2);
                        } else if two_safety1 && !two_safety2 {
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, false, only2);
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, true, only2);
                        } else if !two_safety1 && two_safety2 {
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, only1, false);
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, only1, true);
                        } else {
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, false, false);
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, false, true);
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, true, false);
                            Self::update_type_vec(&mut type_vec, caller_type, callee_type, true, true);
                        }
                    }
                }
                if function_unsafe{
        ...
        println!("total unsafe api: {:?}",func_count);
    }

    // (first_flag, second_flag): if this vec contains two types of cons safety, 
    ...
    pub fn start_check_unsafe_cons(&self) {
        let related_items = RelatedFnCollector::collect(self.tcx);
        let hir_map = self.tcx.hir();
        let mut uncons = 0;
        for (_, &ref vec) in &related_items {
            for (body_id, span) in vec{
                let (function_unsafe, _block_unsafe) = ContainsUnsafe::contains_unsafe(self.tcx, *body_id);
                let def_id = hir_map.body_owner_def_id(*body_id).to_def_id();
                if function_unsafe{
                    if self.get_type(def_id.clone()) == 0 {
                        uncons = uncons + 1;
